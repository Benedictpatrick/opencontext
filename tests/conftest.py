...
stderr. It is teardown noise from the framework, not a fault in OpenContext or in
...
def _isolate_environment(monkeypatch):
    ...
    A developer with OPENCONTEXT_UPSTREAM or OPENCONTEXT_TOKENIZER exported would
    ...
    """
    for variable in (
        "OPENCONTEXT_TOKENIZER",
        "OPENCONTEXT_API_KEY",
        "OPENCONTEXT_EXPOSE_CONTENT",
        "OPENCONTEXT_UPSTREAM_API_KEY",
    ):
    ...
    monkeypatch.setenv("OPENCONTEXT_UPSTREAM", "http://127.0.0.1:9/v1")

    from opencontext.core import tokens
